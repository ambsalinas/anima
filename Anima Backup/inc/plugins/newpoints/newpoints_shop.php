...
function newpoints_shop_postbit(&$post)
{
	...
	foreach ($postbit_items_cache as $iid => $item)
	{
		...
		if ($item['icon'] != '')
			$shop_items .= '<a href="'.$mybb->settings['bburl'].'/newpoints.php?action=shop&amp;shop_action=view&amp;iid='.$iid.'"><img src="'.$mybb->settings['bburl'].'/'.$item['icon'].'" title="'.htmlspecialchars_uni($item['name']).'"></a> ';
		else
